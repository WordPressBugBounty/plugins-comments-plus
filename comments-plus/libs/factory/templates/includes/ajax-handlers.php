...
function wbcr_factory_templates_134_subscribe($plugin_instance)
{
	...
	if ( is_wp_error($response) ) {
		wp_send_json_error( array( 'error_message' => $response->get_error_message() ) );
	}
 ...
 *
 */
function wbcr_factory_templates_134_check_license($plugin_instance)
{
	...
	if( empty($action) || !in_array($action, ['activate', 'deactivate', 'sync', 'unsubscribe']) ) {
		wp_send_json_error(['error_message' => __('Licensing action not passed or this action is prohibited!', 'wbcr_factory_templates_134')]);
		die();
	}
		...
		switch( $action ) {
			case 'activate':
				if( empty($license_key) || strlen($license_key) > 32 ) {
					wp_send_json_error(['error_message' => __('License key is empty or license key too long (license key is 32 characters long)', 'wbcr_factory_templates_134')]);
				} else {
					$plugin_instance->premium->activate($license_key);
					$success_message = __('Your license has been successfully activated', 'wbcr_factory_templates_134');
				}
				break;
			case 'deactivate':
				$plugin_instance->premium->deactivate();
				$success_message = __('The license is deactivated', 'wbcr_factory_templates_134');
				break;
			case 'sync':
				$plugin_instance->premium->sync();
				$success_message = __('The license has been updated', 'wbcr_factory_templates_134');
				break;
			case 'unsubscribe':
				$plugin_instance->premium->cancel_paid_subscription();
				$success_message = __('Subscription success cancelled', 'wbcr_factory_templates_134');
				break;
		}
	} catch( Exception $e ) {
